...
namespace
{
    auto calculateFileKey(const std::filesystem::path& filename) -> std::pair<std::array<char, 11>, uint64_t>
    {
        uint64_t staticKey = 0x1415926535897932;
        uint64_t dynamicKey{};

        if (filename.filename().string().starts_with("slot_"))
...
namespace mvgltools::savefile
{
    ...
    void decryptSaveFile(const std::filesystem::path& source, const std::filesystem::path& target)
    {
        ...
        auto size          = static_cast<uint32_t>(length);
        size_t offset      = 0;
        ...
        { // rotate bits step
            ...
            auto initialVector                     = (uint64_t)((magic * val) >> 0x4E);
            auto rotateParameter                   = (uint32_t)(((uint32_t)val) - (initialVector * 0x7FED));
        ...
        { // xor and math step
            ...
            auto tmp                                = (uint64_t)((val * magic2) >> 64);
            auto init = (uint32_t)((val) - (((((val - tmp) >> 1) + tmp) >> 0xF) * 0xCFF7));
            ...
            for (uint32_t i = 0; i < size; i++)
            {
                ...
                constexpr uint64_t localMagic1 = 0xAB8F69E3;
                constexpr uint64_t localMagic2 = 0x2E8BA2E9;
